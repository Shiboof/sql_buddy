class Owner:
    ...
    def find_user_and_lockboxes(self, connection):
        ...
        2. Count how many serial_numbers are assigned to that user_id.
        ...
        try:
            # Step 1: Get the user_id from Lockbox_Current
            ...
            cursor.execute(query_lockboxes, (user_id,))
            lockbox_data = cursor.fetchone()

            if not lockbox_data or lockbox_data[0] == 0:
                return f"No lockboxes found for user_id {user_id}."

            lockbox_count, serial_numbers = lockbox_data

            # Step 3: Retrieve user details from dbo.[User]
            cursor.execute(query_user_details, (user_id,))
            user_details = cursor.fetchone()

            if not user_details:
                return f"No user details found for user_id {user_id}."
